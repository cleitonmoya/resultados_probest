...
\begin{definition}[Definições básicas de funções] Seja $f\colon\Omega_1 \to \Omega_2$
	\begin{outline}[]
		...
		\1 Imagem inversa: \(\inv{f}(B) \coloneq \{\omega_1\in\Omega_1: f(\omega_1) \in B\}\)
...
\begin{theorem}[Propriedades de funções e imagens inversas] Seja \( f\colon\Omega_1 \to \Omega_2 \),  \(\mathmbox{A,B,(A_n)_{n\geq1} \subset \Omega_2}\) e \(C \subset \Omega_1\):
	\begin{outline}[enumerate]
		\1 $\inv{f}$ é bijetora
		\1 $\inv{\left(\inv{f}\right)} = f$
		\1 \(A \subset B \implies \inv{f}(A) \subset \inv{f}(B)\) 
		\1 \( \inv{f}(A\cup B) = \inv{f}(A)\cup \inv{f}(B) \)
		\1 \( \inv{f}(A\cap B) = \inv{f}(A)\cap \inv{f}(B) \)
		\1 \(\inv{f}\left( \bigcup_{n=1}^\infty A_n \right) = \bigcup_{n=1}^\infty \inv{f}(A_n) \)
		\1 \(\inv{f}\left( \bigcap_{n=1}^\infty A_n \right) = \bigcap_{n=1}^\infty \inv{f}(A_n) \)
		\1 \(\inv{f}(\cp{A}) = \cp{\left(\inv{f}(A)\right)} \)
		\1 \( C \subset \inv{f} \left( f(C) \right) \)
		\1 \(f\left(\inv{f}(A)\right) \subset A \)
	\end{outline}
\end{theorem}
...
\begin{theorem}[Propriedades de composições de funções] Seja $f\colon\Omega_1 \to \Omega_2$ e $g\colon\Omega_2 \to \Omega_3$:
	\begin{outline}[enumerate]
		...
		\1 \( f \text{ e } g \text{ são bijetoras } \implies g\circ f \text{ é bijetora} \) e \( \inv{\left(g \circ f\right)} = \inv{f}\circ\inv{g}\)
		\1 \( \inv{\left(g \circ f\right)}(U) = \inv{f}(\inv{g}(U))\), para $U\subset \Omega_3$
	\end{outline}
\end{theorem}
...
\begin{theorem}[2.1]
	Se $\Acal_2$ for uma \sig-álgebra de subconjuntos de $\Omega_2$, então $\inv{X}(\Acal_2)$ é uma \sig-álgebra de subconjuntos de $\Omega_1$.
\end{theorem}

\begin{definition}[2.1 Transformação mensurável]
	Sejam $(\Omega_1, \Acal_1)$ e $(\Omega_2, \Acal_2)$ dois espaços mensuráveis e $X\colon\Omega_1\to\Omega_2$. Dizemos que $X$ é $\Acal_1\backslash\Acal_2$ mensurável se $\inv{X}(\Acal_2) \subset \Acal_1$, isto é,
	\begin{equation}
		\begin{aligned}
			[X\in B] &= \inv{X}(B)\in\Acal_1, \quad \text{para todo } B \in \Acal_2 \\
...
\begin{definition}[2.2 \sig-álgebra \underline{gerada} por $X$ em $\Omega_1$]
	A \sig-álgebra $\inv{X}(\Acal_2)$ é a menor \sig-álgebra $\Acal_1$ de subconjuntos de $\Omega_1$ que torna $X$ uma transformação $\Acal_1\backslash\Acal_2$ mensurável e é denominada \textit{\sig-álgebra gerada} por $X$:
	\begin{equation}
		\sigma(X) \coloneq \inv{X}(\Acal_2) = \{\inv{X}(B): B \in \Acal_2\}
	\end{equation}
\end{definition}
...
\begin{theorem}[2.4 \sig-álgebra \underline{induzida} por $X$ em $\Omega_2$]
	Seja $X\colon\Omega_1\to\Omega_2$ e $\Acal_1$ uma \sig-álgebra de subconjuntos de $\Omega_1$. A classe $X_*(\Acal_1) \coloneq \{ B \subset \Omega_2: \inv{X}(B) \in \Acal_1 \}$ é uma \sig-álgebra de subconjuntos de $\Omega_2$ que será denominada \textit{\sig-álgebra induzida} por $X$ à partir de $\Acal_1$.
\end{theorem}
...
\begin{theorem}[2.2] 
	...
	\begin{equation}
		\inv{X}(\sigma(\Ccal_2)) = \sigma(\inv{X}(\Ccal_2))
	\end{equation}
\end{theorem}

\begin{theorem}[Condição suficiente para mensurabilidade]
	Sejam $(\Omega_1, \Acal_1)$ e $(\Omega_2, \Acal_2)$ dois espaços mensuráveis e $X\colon \Omega_1\to\Omega_2$. Se $\inv{X}(\Ccal_2) \subset \Acal_1$, onde $\Ccal_2$ é a classe que gera $\Acal_2$, então $X$ é $\Acal_1\backslash\Acal_2$ mensurável.
\end{theorem}
...
\begin{theorem}[2.5 Probabilidade induzida ou distribuição de X]
	Considere $(\Omega_1, \Acal_1, \P_1)$ um espaço de probabilidade, $\Omega_2 \neq \emptyset$ e   $\Acal_2 = \{ B \subset \Omega_2: \inv{X}(B) \in \Acal_1 \}$ a \sig-álgebra induzida por $X$. Defina $\P_2\colon\Acal_2 \to [0,1]$ como
	\begin{equation}
...
\begin{definition}[Variável aleatória]
	...
	\begin{equation}
		\begin{aligned}
			[X\in B] &= \inv{X}(B) \in \Acal \quad \text{para todo } B \in \Boe \\
...
\begin{definition}[Notações em Probabilidade] Sejam $(\Omega_1, \Acal_1)$ e $(\Omega_2, \Acal_2)$ espaços mensuráveis. As seguintes notações são usuais:
	\begin{outline}[enumerate]
		\1 $[X \in B] \coloneq \inv{X}(B)$, para $B \in \Acal_2$
		\1 $[X = x] \coloneq \inv{X}(\{x\})$, para ${x} \in \Acal_2$
		\1 $[X \leq x] \coloneq \inv{X}((-\infty, x])$, para $(-\infty, x] \in \Acal_2$
		\1 \( f\colon \Re\to\Re : [f\leq y] \coloneq \inv{f}((-\infty, y]) = \{x \in\Re:f(x)\leq y \} \)
		\1 \( [X \leq x, Y \leq y] \coloneq \inv{X}((-\infty, x])\cap\inv{Y}((-\infty, y])\)
		\1 \([X<Y] = [X-Y<0] \coloneq \inv{Z}((-\infty, 0))\), onde $Z=X-Y$
		\1 \(F(x) = \P((-\infty, x])\)
		\1 \(F_X(x) = \P[X\leq x] = \P(\inv{X}(-\infty,x]) = \P_X((-\infty,x]) \equiv \P((-\infty, x]) = F(x)\)
...
\begin{theorem}
		...
		\begin{outline}[enumerate]
			...
			\([X=+\infty] = \inv{X}(\{+\infty\}) \in \Acal\), \\
			\([X=-\infty] = \inv{X}(\{-\infty\}) \in \Acal\);
			\1 \([X \in I] = \inv{X}(I) \in \Acal\) para todo $I \in \Ical$, em que $\sigma(\Ical)=\Boe$.
		\end{outline}
\end{theorem}
...
\begin{definition}
	...
	\begin{outline}[enumerate]
		...
		\1 \textit{Cilindro unidimensional de base $A_i$}: \( \inv{\pi_i}(A_i) = \{ \omega\in\Omega : \pi_i(\omega)\in A_i\} \) 
		\1 \textit{Classe dos cilindros unidimensionais}: \( \Ccal = \{\inv{\pi_i}(A_i) : A_i \in \Acal, i=1,\ldots,n\}\)
