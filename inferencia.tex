...
\begin{theorem}[Qui-quadrado]
	\leavevmode
	\begin{outline}[enumerate]
		\1 Se $Z \sim \normdist{0,1}$, então $Z^2 \sim \chi_1^2$;
		
		\1 Se $X_1, X_2, \ldots, X_n$ são v.a.'s independentes tais que $X_i \sim \chi_{r_i}^2$, então $\sum_{i=1}^{n} X_i \sim \chi_r^2$, em que $r = \sum_{i=1}^{n} r_i$.
	\end{outline}
\end{theorem}

\begin{theorem}[Distribuição normal]
...
\begin{definition}[Princípio da suficiência]
	Se $T$ for uma estatística suficiente para $\theta$, e se duas amostras observadas, $\vect{x}$ e $\vect{y}$, forem tais que $T(\vect{x}) = T(\vect{y})$, então as inferências a respeito de $\theta$ deverão ser as mesmas, independentemente de se usar $\vect{x}$ ou $\vect{y}$.
	
	\begin{note}
		Como consequência, qualquer inferência sobre $\theta$ deverá depender da amostra $\vect{X}$ apenas através de $T(\vect{X})$.
	\end{note}
\end{definition}

\begin{definition}[Estatística suficiente]
	Uma estatística $T(\vect{X})$ é chamada de estatística suficiente para $\theta$ se a distribuição condicional da amostra $\vect{X}$, dado o valor de $T(\vect{X})$, não depender de $\theta$.
\end{definition}

\begin{theorem}[Razão para suficiência]
	Seja $p(\vect{x} | \theta)$ a f.p. (ou f.d.p.) conjunta de uma amostra $\vect{X}$, e $q(t | \theta)$ a f.p. (ou f.d.p.) de uma estatística $T(\vect{X})$. Então $T(\vect{X})$ será uma estatística suficiente para $\theta$ se, para todo $\vect{x} \in \mathcal{X}$, a razão
	\[
	\frac{p(\vect{x} | \theta)}{q(T(\vect{x}) | \theta)}
	\]
	for constante como função de $\theta$.
\end{theorem}

\begin{theorem}[Critério da fatoração]
	Seja $f(\vect{x} | \theta)$ a f.p. (ou f.d.p.) conjunta de uma amostra $\vect{X}$. Uma estatística $T(\vect{X})$ é uma estatística suficiente para $\theta$ se, e somente se, existirem funções $g(t | \theta)$ e $h(\vect{x})$ de modo que, para todo $\vect{x} \in \mathcal{X}$ e todo $\theta \in \Theta$,
	\[
	f(\vect{x} | \theta) = g(T(\vect{x}) | \theta)h(\vect{x}).
	\]
\end{theorem}
...
\begin{theorem}[Suficiência na família exponencial]
	...
	\[
	f(x | \vect{\theta}) = \exp\left(\sum_{k=1}^{s} \eta_k(\vect{\theta})t_k(x)\right) a(\vect{\theta})h(x),
	\]
	em que $\vect{\theta} = (\theta_1, \theta_2, \ldots, \theta_d)$, $d \leq s$. Então,
	\[
	T(\vect{X}) = \left(\sum_{i=1}^{n} t_1(X_i), \sum_{i=1}^{n} t_2(X_i), \ldots, \sum_{i=1}^{n} t_s(X_i)\right)
	\]
	é uma estatística suficiente para $\vect{\theta}$.
\end{theorem}

\begin{definition}[Estatística suficiente mínima]
	Uma estatística $T(\vect{X})$ é chamada estatística suficiente mínima se, para qualquer outra estatística suficiente $T'(\vect{X})$, $T(\vect{X})$ puder ser escrita como função de $T'(\vect{X})$.
\end{definition}

\begin{theorem}[Critério para suficiência mínima]
	Seja $f(\vect{x} | \theta)$ a f.p. (ou f.d.p.) conjunta de uma amostra $\vect{X}$. Suponhamos que, para quaisquer duas possíveis amostras observadas, $\vect{x}$ e $\vect{y}$, $f(\vect{x} | \theta)/f(\vect{y} | \theta)$ seja constante como função de $\theta$ se, e somente se, $T(\vect{x}) = T(\vect{y})$. Então $T(\vect{X})$ é uma estatística suficiente mínima.
\end{theorem}

\begin{definition}[Estatística ancilar]
	Uma estatística $S(\vect{X})$ cuja distribuição não depende de $\theta$ é chamada de estatística ancilar.
	
	\begin{note}
		Uma estatística $S(\vect{X})$ é dita ser ancilar de primeira ordem se $\E_\theta[S(\vect{X})]$ não depende de $\theta$.
	\end{note}
\end{definition}

\begin{definition}[Estatística completa]
	Seja $f(t | \theta)$ uma família de f.d.p.'s (ou f.p.'s) para uma estatística $T(\vect{X})$. A família de distribuições de probabilidade é chamada completa se, para qualquer função $g$,
	\[
	\E_\theta[g(T)] = 0 \implies \P_\theta[g(T) = 0] = 1,
	\]
	para todo $\theta \in \Theta$. De forma equivalente, $T(\vect{X})$ é chamada de estatística completa.
\end{definition}

\begin{theorem}[Basu]
	Se $T(\vect{X})$ for uma estatística suficiente mínima e completa, então $T(\vect{X})$ será independente de toda estatística ancilar.
\end{theorem}
...
\begin{theorem}[Estatísticas completas na família exponencial]
	...
	\[
		f(x | \vect{\theta}) = \exp\left(\sum_{k=1}^{s} \eta_k(\vect{\theta})t_k(x)\right) a(\vect{\theta})h(x),
	\]
	em que $\vect{\theta} = (\theta_1, \theta_2, \ldots, \theta_s)$. Então, a estatística
	\[
		T(\vect{X}) = \left(\sum_{i=1}^{n} t_1(X_i), \sum_{i=1}^{n} t_2(X_i), \ldots, \sum_{i=1}^{n} t_s(X_i)\right)
	\]
...
\begin{definition}[Função de verossimilhança]
	...
	\[
	L(\theta | \vect{x}) = f(\vect{x} | \theta).
	\]
	Qualquer estimador $\hat{\theta} \equiv \hat{\theta}_n(\vect{X}) \in \Theta$ tal que
	\[
	L(\hat{\theta} | \vect{X}) = \sup_{\theta \in \Theta} L(\theta | \vect{X})
	\]
...
\begin{theorem}[Relação EMV e Suficiência]
	Seja $L(\theta | \vect{x})$ a função de verossimilhança da amostra $X_1, X_2, \ldots, X_n$, e seja $T = T(\vect{X})$ uma estatística suficiente para $\theta$. O EMV de $\theta$, se existir, será função de $T$.
\end{theorem}
...
\begin{theorem}[EMV na família exponencial]
	...
	\[
	f(\vect{x} | \eta) = \exp\left(\eta \sum_{i=1}^{n} T(x_i) - nA(\eta)\right) h^*(\vect{x}).
	\]
	...
	\begin{note}
		...
		\[
		f(\vect{x} | \vect{\theta}) = \exp\left(\sum_{k=1}^{s} \eta_k(\vect{\theta})T_k(\vect{x}) - B(\vect{\theta})\right) h(\vect{x}).
		\]
	\end{note}
\end{theorem}
...
\begin{theorem}[Desigualdade de Cramér-Rao (Teorema 1)]
	Sejam $X_1, X_2, \ldots, X_n$ uma amostra com f.d.p. conjunta $f(\vect{x} | \theta)$ e $W(\vect{X})$ um estimador que satisfaz
	\[
	\frac{d}{d\theta} \E_\theta[W(\vect{X})] = \int_{\mathcal{X}} \frac{\partial}{\partial\theta} [W(\vect{x})f(\vect{x} | \theta)]d\vect{x} \quad \text{e} \quad \Var_\theta[W(\vect{X})] < \infty.
	\]
	Então,
	\[
	\Var_\theta[W(\vect{X})] \geq \frac{\left(\frac{d}{d\theta} \E_\theta[W(\vect{X})]\right)^2}{\E_\theta\left[\left(\frac{\partial}{\partial\theta} \log f(\vect{X} | \theta)\right)^2\right]}.
	\]
	...
	\begin{note}
		...
		\begin{outline}[enumerate]
			...
			\1 A quantidade
			\[
			\frac{\left(\frac{d}{d\theta} \E_\theta[W(\vect{X})]\right)^2}{\E_\theta\left[\left(\frac{\partial}{\partial\theta} \log f(\vect{X} | \theta)\right)^2\right]},
			\]
			exibida no Teorema 1, é conhecida como limite inferior de Cramér-Rao (LICR).
		
			\1 A quantidade
			\[
			\E_\theta\left[\left(\frac{\partial}{\partial\theta} \log f(\vect{X} | \theta)\right)^2\right]
			\]
...
\begin{theorem}[Desigualdade de Cramér-Rao -- caso i.i.d. (Corolário 1)]
	...
	\[
	\Var_\theta[W(\vect{X})] \geq \frac{\left(\frac{d}{d\theta} \E_\theta[W(\vect{X})]\right)^2}{n\E_\theta\left[\left(\frac{\partial}{\partial\theta} \log f(X | \theta)\right)^2\right]}.
	\]
\end{theorem}
...
\begin{theorem}[Corolário 2]
	Sejam $X_1, X_2, \ldots, X_n$ uma A.A. de uma população $X$ com f.d.p. (ou f.p.) $f(x | \theta)$, em que $f$ satisfaz as condições do Teorema 1. Suponhamos que $L(\theta | \vect{x}) = \prod_{i=1}^{n} f(x_i | \theta)$ denote a função de verossimilhança. Se $W(\vect{X})$ for um estimador para $\tau(\theta)$, então $W(\vect{X})$ atingirá o limite inferior de Cramér-Rao se, e somente se,
	\[
	\frac{\partial}{\partial\theta} \log L(\theta | \vect{x}) = a(\theta)[W(\vect{x}) - \tau(\theta)],
	\]
...
\begin{theorem}[Teorema 5]
	...
	\begin{note}
		O teorema acima diz que, na presença de estatísticas suficientes e completas, se pudermos determinar qualquer estimador não viesado de $\tau(\theta)$, podemos determinar o melhor ENV. Basicamente, se $T$ for uma estatística suficiente e completa para $\theta$ e $h(\vect{X})$ um ENV de $\tau(\theta)$, então $\phi(T) = \E[h(\vect{X}) | T]$ será o melhor estimador não viesado de $\tau(\theta)$.
	\end{note}
\end{theorem}